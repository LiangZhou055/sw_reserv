...
<div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
  <div class="flex items-center justify-between gap-4">
    <h3 class="text-base font-bold text-slate-900">Reservation Detail - {{ strtoupper($store->code) }}</h3>
    <a class="inline-flex rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100 transition"
       href="{{ route('admin.stores.reservations.index', ['code' => $store->code, 'start_date' => $startDate, 'end_date' => $endDate, 'page' => $page]) }}">
      Back to list
    </a>
  </div>
...
<div class="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
  <div class="flex items-center justify-between gap-4">
    ...
    @if(!empty($smsTableName))
      <span class="inline-flex items-center rounded-full bg-indigo-100 px-3 py-1 text-xs font-bold text-indigo-700">{{ $smsLogs->count() }} rows</span>
    @endif
  </div>
